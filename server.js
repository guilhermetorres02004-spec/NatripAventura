...
app.use((req, res, next) => {
  res.setHeader('Content-Security-Policy',
    "default-src 'self'; script-src 'self' 'unsafe-inline' https://www.paypal.com https://www.sandbox.paypal.com; style-src 'self' 'unsafe-inline' https://www.gstatic.com https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; img-src 'self' data: https:; connect-src 'self' https: wss:;");
  next();
});
...
console.log('Database configuration:', {
  hasConnectionString: !!NORMALIZED_DATABASE_URL,
  ssl: dbConfig.ssl,
...
// when running on Vercel (or any other serverless platform) we don't
// call listen directly – the platform will invoke the exported `app`
// handler.  Locally we still start the HTTP server so `npm start` works.
